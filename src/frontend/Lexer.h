...
#define LEXEME_LIST E(Error),E(ID),E(DIGIT),E(DECIMAL),E(STRING),E(CHAR),\
    ...
    E(end_if),E(end_function),E(end_sub),E(end_module),E(end_class),E(end_type),E(end_enum),E(end_select),E(end_init),E(end_operator),E(end_interface), \
    E(import),E(declare),E(lib),E(alias),E(init),E_(operator), \
    E(impl),E(extend),E(interface),E_(protected)
